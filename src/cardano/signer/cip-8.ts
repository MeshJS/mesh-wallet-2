...
class CoseSign1 {
  ...
  createSigStructure(externalAad: Buffer = Buffer.alloc(0)): Buffer {
  ...
  verifySignature({
    ...
    return publicKey.verify(
      ...
      HexBlob(Buffer.from(this.createSigStructure(externalAad)).toString("hex"))
    );
  }
  ...
  getSignature(): Buffer | undefined {
    return this.signature ? Buffer.from(this.signature.bytes) : undefined;
  }
